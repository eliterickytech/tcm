...
if ($data_token['session'] == session_id()) {
    ...
    if(!$user_group == "1"){
        ...
        //###############
      $get_all_parts_collection = "SELECT * FROM collections RIGHT JOIN collections_pieces ON collections.collection_hash = collections_pieces.collection_master_hash WHERE collections.collection_hash = '$collection_hash' ";
      if(!$get_all_parts_collection = get_all_data($get_all_parts_collection, $connection)){
        echo "Error get all parts the collection";
        exit();
      } else {
        $result = array();
        foreach($get_all_parts_collection as $item) {
            $result[] = array(
...
<html>
  ...
  <body onload="onLoad()" class="access">
    <div class="add-collection-2-operador-">
      ...
      <div class="add-collection1">Add collection 3x3</div>
      <section>
        <!-- START MINIATURE -->
          <button id="upload_miniature_button" style="border:none; background-color: #FFF;">
            <img id="collection_miniature" class="add-collection-2-operador-child2" src="./img/file-minier.png">
          </button>
        <form action="#" method="POST">
          <input class="add-collection-2-operador-child3" type="text" name="miniature_collection_piece_description" id="miniature_collection_piece_description" placeholder="Shortened name for home screen display" autocomplete="off">
        ...
        <div class="grid-container">          
             <?php foreach($result as $key => $value):?> 
              ...
              <?php foreach($value as $item => $data):?>
                  <?php if($item == "collection_piece_image"):?>
                    <div class="column-1">
                      <span class="labelImage">Part <?php echo $part_i;?></span>
                      <?php                     
                        $myFile = $data;
                        $fh = fopen($myFile, 'r');
                        $data_file = fread($fh, filesize($myFile));
                        fclose($fh);
                      ?>
                      <img id="imageid_<?php echo $part_i;?>" name="imageid_<?php echo $part_i;?>" src="<?php echo(isset($data_file) ? $data_file : './img/file-minier.png');?>" alt="Imagem">
                    </div>
                    <div class="column-2">
                      <textarea id="description_<?php echo $part_i;?>" name="description_<?php echo $part_i;?>" rows="2" placeholder="Description part of collection" autocomplete="off"></textarea>
                    </div>
                  <?php endif;?>
              <?php endforeach;?>
                <?php $part_i++;?>
      ...
      <input type="radio" class="ellipse-icon" name="collection_available_now" value="1">
      <div class="available-at-">Available at <input type="date" style="border: none; font-size: 7px; width: 30%;" name="collection_date_available" id="collection_date_available" value="<?php echo(isset($collection_date_available) ? $collection_date_available : '____-__-__') ;?>" placeholder="____-__-__"> <input type="time" style="border: none; font-size: 7px;  width: 20%;" name="collection_time_available" id="collection_time_available" value="<?php echo(isset($collection_time_available) ? $collection_time_available : '__:__');?>"></div>
      <input type="radio" class="add-collection-2-operador-child11" name="collection_available_now" value="2">
      <div class="miniature">Miniature:</div>
