...
if ($data_send['session'] == session_id()) {
    ...
    $sSQL = "SELECT * FROM users WHERE user_password = '$user_password' and user_email = '$user_email' LIMIT 0,1";
        ...
        if ($rowCount > 0) {
            while ($dados = mysqli_fetch_array($query)) {
                ...
                if (Request_DB($insert_sql, $connection)) {
                    ...
                    //##################################################################################
                    if($user_group == "1"){
                        $url_redirect = "<script type='text/javascript'>window.location = 'https://thechefmelo.com/my.php?token=" . $access_token . "'</script>";
                    } else {
                        $url_redirect = "<script type='text/javascript'>window.location = 'https://thechefmelo.com/home.php?token=" . $access_token . "'</script>";
                    }
                } else {
